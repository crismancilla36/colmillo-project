...
<div class="card">
  ...
  <form class="card-body pt-2">
    ${
      ...
      <span class="flex-grow">${error.tarea}</span>
    </div>`
        : ''
    }
...
// Renders a tarea to the page
const addtarea = (tarea) => {
  // The user that sent this tarea (added by the populate-user hook)
  const { user = {} } = tarea
  const lista = document.querySelector('#lista')
  // Escape HTML to prevent XSS attacks
  const text = escapeHTML(tarea.text)

  if (lista) {
    lista.innerHTML += `<div class="lista lista-start py-2">
      <div class="lista-image avatar">
        <div class="w-10 rounded-full">
          <img src="${user.avatar}" />
        </div>
      </div>
      <div class="lista-bubble">${text}</div>
    </div>`

    // Always scroll to the bottom of our tarea list
...
const showLogin = () => {
  ...
  // Shows the lista page
  const showlista = async () => {
    document.getElementById('app').innerHTML = listaTemplate()
  
    // Find the latest 25 tareas. They will come with the newest first
    const tareas = await client.service('tareas').find({
      query: {
        $sort: { createdAt: -1 },
        $limit: 25
      }
    ...
    // We want to show the newest tarea last
    tareas.data.reverse().forEach(addtarea)
...
const getCredentials = () => {
  ...
  const login = async (credentials) => {
    try {
      ...
      // If successful, show the lista page
      showlista()
    } catch (error) {
  ...
  // "Send" tarea form submission handler
  addEventListener('#send-tarea', 'submit', async (ev) => {
    // This is the tarea text input field
    const input = document.querySelector('[name="text"]')
  
    ev.preventDefault()
  
    // Create a new tarea and then clear the input field
    await client.service('tareas').create({
      text: input.value
    })
  ...
  // Listen to created events and add the new tarea in real-time
  client.service('tareas').on('created', addtarea)
  
  // We will also see when new users get created in real-time
  client.service('users').on('created', addUser)
  
  // Call login right away so we can show the lista window
